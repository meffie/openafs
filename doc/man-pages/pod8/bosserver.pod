...
=item B<-skip-root-check>

Allow the BOS Server (and thus the processes started by the BOS Server) to run
as a regular local user.  By default, the BOS Server process must be run by the
local C<root> user.
...
superuser C<root>, unless the B<-skip-root-check> option is specified.
