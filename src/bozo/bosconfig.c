 ...
 * @param pbuffer   output buffer containing file contents
 * @return 0 on success
 */
static int
slurp_file(const char *filename, char **pbuffer)
{
    int code;
    FILE *fp;
    struct stat st;
    ...
    fp = fopen(filename, "r");
    if (fp == NULL) {
	/* Assume a cold startup if the file was not found. */
	if (errno == ENOENT) {
	    buffer = strdup("");
	    if (buffer == NULL) {
	    ...
	    bozo_Log("File '%s' not found; using empty configuration.", filename);
	    goto done;
	}
    ...
    if (code < 0) {
	bozo_Log("Failed to stat file %s; errno %d\n", filename, errno);
	code = errno;
	goto fail;
    }
	...
	bozo_Log("Failed to read file %s; errno %d\n", filename, errno);
	code = errno;
	free(buffer);
	goto fail;
    }
...
ReadBozoFile(const char *file)
{
    ...
    /* Read the BosConfig into a buffer. */
    code = slurp_file(file, &buffer);
    if (code != 0)
	goto done;

    /* Parse the contents by line and save the values. */
    text = strtok_r(buffer, "\n", &save);
    while (text != NULL) {
	for (p = parsers; p->tag; p++) {
	    if (state == p->state && strncmp(text, p->tag, p->len) == 0) {
		code = p->parse(text + p->len, p->rock, file, line);
		if (code != 0)
		    goto done;
		state = p->next;
		break;
