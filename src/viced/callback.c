...
BreakCallBack(struct host *xhost, AFSFid * fid, int flag)
{
	...
	ViceLog(7,
		("BCB: BreakCallBack(Host %p all but %s, (%u,%u,%u))\n",
		 xhost, h_Host2str(xhost, &hoststr),
		...
		if (!thishost) {
		    ...
		    ViceLog(7,
			    ("BCB: %p (%s) is down; delaying break call back\n",
			     thishost, h_Host2str(thishost, &hoststr)));
	...
	if (cbi) {
		...
		ViceLog(8,
			("CCB: deleting timed out call back %x (%s), (%" AFS_VOLID_FMT ",%u,%u)\n",
			 h_itoh(cb->hhead)->z.addr.sin_addr.s_addr,
				     ...
				     struct AFSCBFids *afidp)
{
	...
	if (host->z.addr.sin_addr.s_addr == host->z.interface->interface[i].addr.sin_addr.s_addr
	   && host->z.addr.sin_port == host->z.interface->interface[i].addr.sin_port)
	    continue;

	interfaces[j] = host->z.interface->interface[i];
	conns[j] =
	    rx_NewConnection(interfaces[j].addr.sin_addr.s_addr,
			     interfaces[j].addr.sin_port, 1, sc, 0);
	...
	if (!multi_error) {
	    ...
	    host->z.addr.sin_addr.s_addr = interfaces[multi_i].addr.sin_addr.s_addr;
	    host->z.addr.sin_port = interfaces[multi_i].addr.sin_port;
	    ...
	    ViceLog(125,
		    ("multibreakcall success with addr %s\n",
		     h_AddrPort2str(&interfaces[multi_i], &hoststr)));
	    H_UNLOCK;
	    multi_Abort;
	}
    }
	...
	if (!multi_error) {
	    ...
	    ViceLog(125,
            ...
            if (multi_error == 1) {
                ...
                H_LOCK;
                removeInterfaceAddr_r(host, &interfaces[multi_i].addr);
                H_UNLOCK;
            }
        }
#ifdef AFS_DEMAND_ATTACH_FS
