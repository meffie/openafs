...
static int h_Lookup_r(struct sockaddr_in *addr, struct host **hostp);
static void h_AddHostToAddrHashTable_r(struct sockaddr_in *addr, struct host * host);
static int h_DeleteHostFromAddrHashTable_r(struct sockaddr_in *addr, struct host *host);
...
h_HashIndex(struct sockaddr_in *addr)
{
    return ntohl(addr->sin_addr.s_addr) & (h_HASHENTRIES-1);
}

static_inline afs_uint32
h_UuidHashIndex(afsUUID *uuidp)
{
    return ((int)(afs_uuid_hash(uuidp))) & (h_HASHENTRIES-1);
}

static_inline int
h_equal(struct sockaddr_in *a, struct sockaddr_in *b)
{
...
/* format the network address and port from a AddrHashChain
 * returns buffer "<netaddr>:<port>" */
...
AddrHashChain2str(struct h_AddrHashChain *chain, struct rx_inet_fmtbuf *buf)
{
    ...
    snprintf(buf->buffer, sizeof(buf->buffer), "%s:%hu",
	     rx_inet2str(chain->addr.sin_addr.s_addr, &inetfmtbuf),
	     ntohs(chain->addr.sin_port));
    return buf->buffer;
}
...
static int
hpr_GetHostCPS(struct sockaddr_in *addr, prlist *CPS)
{
    afs_int32 code;
    afs_int32 over;
    struct ubik_client *uclient;
    afs_uint32 host = ntohl(addr->sin_addr.s_addr);
...
h_gethostcps_r(struct host *host, afs_int32 now)
{
    ...
    H_UNLOCK;
    code = hpr_GetHostCPS(&host->z.addr, &host->z.hcps);
    H_LOCK;
    if (code) {
	...
	if (code < 0 || code == UNOQUORUM || code == UNOTSYNC) {
	    ...
	    ViceLog(0,
		    ("Warning:  GetHostCPS failed (%d) for %p (%s); will retry\n",
		     code, host, h_Host2str(host, &hoststr)));
	} else {
	    host->z.hcpsfailed = 0;
	    ViceLog(1,
...
void
h_flushhostcps(struct sockaddr_in *addr)
{
    struct host *host;

    H_LOCK;
    h_Lookup_r(addr, &host);
    if (host) {
	host->z.hcpsfailed = 1;
	h_Release_r(host);
    }
...
h_Alloc_r(struct rx_connection *r_con)
{
    ...
    host->z.addr.sin_family = AF_INET;
    host->z.addr.sin_addr.s_addr = rxr_HostOf(r_con);
    host->z.addr.sin_port = rxr_PortOf(r_con);
    ...
    if (host->z.addr.sin_port == consolePort)
	host->z.Console = 1;
     ...
     */
    newHostAddr_HBO = (afs_uint32) ntohl(host->z.addr.sin_addr.s_addr);
...
h_SetupCallbackConn_r(struct host * host)
{
	...
	rx_NewConnection(host->z.addr.sin_addr.s_addr, host->z.addr.sin_port, 1, sc, 0);
 ...
 */
static int
h_Lookup_r(struct sockaddr_in *addr, struct host **hostp)
{
    afs_int32 now;
...
h_TossStuff_r(struct host *host)
{
	...
	struct h_hostdebug_fmtbuf hoststr;
	if (wasdeleted) {
	    /* someone grabbed a ref while HOSTDELETED was set; that is bad */
	    ViceLog(0, ("Warning:  h_TossStuff_r failed; Host %p"
			" (%s) was held.\n",
			host, h_Host2Debugstr(host, &hoststr)));
	}
	return;
    }
	...
	if (!(host->z.interface)) {
	    h_DeleteHostFromAddrHashTable_r(&host->z.addr, host);
	} else {
		...
		hostAddrPort = host->z.interface->interface[i];
                 ...
                 * in the hash table.
                 */
                if (hostAddrPort.valid && h_equal(&host->z.addr, &hostAddrPort.addr))
...
h_AddHostToUuidHashTable_r(struct afsUUID *uuid, struct host *host)
{
	...
	if (chain->hostPtr->z.interface &&
	    ...
	    if (GetLogLevel() >= 125) {
		struct h_hostdebug_fmtbuf hostdebugbufs[2];
		ViceLog(125, ("h_AddHostToUuidHashTable_r: host %p (%s) "
			      "already exists as chain %p (%s)\n",
			      host,
			      h_Host2Debugstr(host, &hostdebugbufs[0]),
			      chain->hostPtr,
			      h_Host2Debugstr(chain->hostPtr, &hostdebugbufs[1])));
	    }
	    return;
	}
    }
    ...
    hostUuidHashTable[index] = chain;
     ...
     ViceLog(125,
	     ("h_AddHostToUuidHashTable_r: host %p (%s) added as uuid %s\n",
	      host, h_Host2str(chain->hostPtr, &hoststr),
	      uuid2.buffer));
}
...
h_DeleteHostFromUuidHashTable_r(struct host *host)
{
     ...
     struct h_hostdebug_fmtbuf hostdebugbuf;
	 ...
	 if (uth->hostPtr == host) {
	     ViceLog(125,
		     ("h_DeleteHostFromUuidHashTable_r: host %p (%s)\n",
		      host, h_Host2Debugstr(host, &hostdebugbuf)));
	     *uhp = uth->next;
	     free(uth);
	     return 1;
	 }
     ...
     ViceLog(125,
	     ("h_DeleteHostFromUuidHashTable_r: host %p (%s) not found\n",
	      host, h_Host2Debugstr(host, &hostdebugbuf)));
     return 0;
}
 ...
 */
static int
invalidateInterfaceAddr_r(struct host *host, struct sockaddr_in *addr)
{
    int i;
    int number;
    ...
    ViceLog(125, ("invalidateInterfaceAddr : host %p (%s) addr %s:%d\n",
		  host, h_Host2str(host, &hoststr),
		  afs_inet_ntoa_r(addr->sin_addr.s_addr, hoststr2),
		  ntohs(addr->sin_port)));
    ...
    for (i = 0; i < number; i++) {
	if (h_equal(&interface->interface[i].addr, addr)) {
 ...
 */
static int
removeAddress_r(struct host *host, struct sockaddr_in *addr)
{
    int i;
	...
	if (h_equal(&host->z.addr, addr)) {
	    ...
	    ViceLog(0,
		    ("Removing address that does not belong to host %p (%s).\n",
		     host, h_Host2str(host, &hoststr)));
        }
    } else {
	if (h_equal(&host->z.addr, addr)) {
		...
		if (host->z.interface->interface[i].valid) {
		    ...
		    host->z.addr.sin_family = AF_INET;
		    host->z.addr.sin_addr.s_addr = host->z.interface->interface[i].addr.sin_addr.s_addr;
		    host->z.addr.sin_port = host->z.interface->interface[i].addr.sin_port;
		    h_AddHostToAddrHashTable_r(&host->z.addr, host);
                    break;
                }
            }
	    ...
	    if (i == host->z.interface->numberOfInterfaces) {
                ViceLog(25,
			 ("Removed only address for host %p (%s), no valid alternate interfaces, deleting host.\n",
			   host, h_Host2str(host, &hoststr)));
		host->z.hostFlags |= HOSTDELETED;
                /* addr/port was removed from the hash table */
		host->z.addr.sin_family = AF_INET;
		host->z.addr.sin_addr.s_addr = 0;
		host->z.addr.sin_port = 0;
            } else {
		...
		h_SetupCallbackConn_r(host);
            ...
            invalidateInterfaceAddr_r(host, addr);
        }
    }
...
static void
createHostAddrHashChain_r(int index, struct sockaddr_in *addr, struct host *host)
{
    struct h_AddrHashChain *chain;
    struct rx_inet_fmtbuf hoststr;
    ...
    chain->addr.sin_family = AF_INET;
    chain->addr.sin_addr.s_addr = addr->sin_addr.s_addr;
    chain->addr.sin_port = addr->sin_port;
 ...
 */
static void
reconcileHosts_r(struct sockaddr_in *addr, struct host *newHost,
		 struct host *oldHost)
{
    ...
    ViceLog(125,
	    ("reconcileHosts_r: addr %s:%d newHost %p oldHost %p\n",
	     afs_inet_ntoa_r(addr->sin_addr.s_addr, hoststr), ntohs(addr->sin_port),
	     newHost, oldHost));
    ...
    cb = rx_NewConnection(addr->sin_addr.s_addr, addr->sin_port, 1, sc, 0);
	...
	ViceLog(0,
		("reconcileHosts_r: WhoAreYou not supported for connection (%s:%d), error %d\n",
		 afs_inet_ntoa_r(addr->sin_addr.s_addr, hoststr), ntohs(addr->sin_port), code));
	goto fail;
    }
    if (code != 0) {
	ViceLog(0,
...
static void
h_AddHostToAddrHashTable_r(struct sockaddr_in *addr, struct host *host)
{
    int index;
    struct h_AddrHashChain *chain;
    struct rx_inet_fmtbuf hoststr;
	...
	if (h_equal(&chain->addr, addr)) {
		...
		reconcileHosts_r(addr, host, chain->hostPtr);
		return;
	    }
	}
    }
    createHostAddrHashChain_r(index, addr, host);
}
 ...
 */
int
addInterfaceAddr_r(struct host *host, struct sockaddr_in *addr)
{
    int i;
    int number;
    struct Interface *interface;
    ...
    number = host->z.interface->numberOfInterfaces;
    for (i = 0; i < number; i++) {
	if (host->z.interface->interface[i].addr.sin_addr.s_addr
	     == addr->sin_addr.s_addr
	     && host->z.interface->interface[i].addr.sin_port == addr->sin_port) {
	    ViceLog(125,
		    ("addInterfaceAddr : found host %p (%s) adding %s:%d%s\n",
		     host, h_Host2str(host, &hoststr),
		     afs_inet_ntoa_r(addr->sin_addr.s_addr, hoststr2),
		     ntohs(addr->sin_port), host->z.interface->interface[i].valid ? "" :
		     ", validating"));
    ...
    ViceLog(125, ("addInterfaceAddr : host %p (%s) adding %s:%d\n",
		  host, h_Host2str(host, &hoststr),
		  afs_inet_ntoa_r(addr->sin_addr.s_addr, hoststr2),
		  ntohs(addr->sin_port)));
    ...
    interface->interface[number].addr.sin_family = AF_INET;
    interface->interface[number].addr.sin_addr.s_addr = addr->sin_addr.s_addr;
    interface->interface[number].addr.sin_port = addr->sin_port;
 ...
 */
int
removeInterfaceAddr_r(struct host *host, struct sockaddr_in *addr)
{
    int i;
    int number;
    struct Interface *interface;
    ...
    ViceLog(125, ("removeInterfaceAddr : host %p (%s) addr %s:%d\n",
		  host, h_Host2str(host, &hoststr),
		  afs_inet_ntoa_r(addr->sin_addr.s_addr, hoststr2),
		  ntohs(addr->sin_port)));
    ...
    number = host->z.interface->numberOfInterfaces;
    for (i = 0; i < number; i++) {
	if (interface->interface[i].addr.sin_addr.s_addr
	    == addr->sin_addr.s_addr
	    && interface->interface[i].addr.sin_port == addr->sin_port) {
	    if (interface->interface[i].valid)
		h_DeleteHostFromAddrHashTable_r(addr, host);
	    number--;
	    for (; i < number; i++) {
 ...
 */
static int
ShouldSkipTMAY(struct host *host, int prewait_tmays, struct sockaddr_in *prewait_addr)
{
    int skiptmay = 0;
    ...
    if (!h_equal(&host->z.addr, prewait_addr)) {
...
h_GetHost_r(struct rx_connection *tcon)
{
    ...
    addr.sin_addr.s_addr = rxr_HostOf(tcon);
    addr.sin_port = rxr_PortOf(tcon);
  retry:
    if (cb_in) {
	...
	struct sockaddr_in prewait_addr;
	int skiptmay;
	...
	prewait_addr.sin_family = AF_INET;
	prewait_addr.sin_addr.s_addr = host->z.addr.sin_addr.s_addr;
	prewait_addr.sin_port = host->z.addr.sin_port;
	...
	if (!(host->z.hostFlags & ALTADDR) ||
	    ...
	    h_Unlock_r(host);
	    ViceLog(125,
		    ("Host %p (%s) starting h_Lookup again\n",
		     host, h_Host2str(host, &hoststr)));
	    h_Release_r(host);
	    goto retry;
	}
	...
	if (h_equal(&addr, &host->z.addr)) {
	    ...
	    if (!sc)
                sc = rxnull_NewClientSecurityObject();
            cb_in = rx_NewConnection(addr.sin_addr.s_addr, addr.sin_port, 1, sc, 0);
	...
	if ((code == RXGEN_OPCODE) ||
		...
		if (host->z.interface) {
		    ViceLog(0,
			    ("Host %p (%s) used to support WhoAreYou, deleting.\n",
			     host,
			     h_Host2str(host, &hoststr)));
		    host->z.hostFlags |= HOSTDELETED;
		    host->z.hostFlags &= ~HWHO_INPROGRESS;
		    h_Unlock_r(host);
		    h_Release_r(host);
		...
		if (cb_in) {
			ViceLog(25,
					("Uuid doesn't match connection (%s).\n",
					 rx_Conn2str(tcon, &hoststr)));
			removeAddress_r(host, &addr);
		} else {
		    ViceLog(25,
			    ("Uuid doesn't match host %p (%s).\n",
			     host, h_Host2str(host, &hoststr)));

		    removeAddress_r(host, &host->z.addr);
		}
		host->z.hostFlags &= ~HWHO_INPROGRESS;
		host->z.hostFlags |= ALTADDR;
		h_Unlock_r(host);
		...
		cb_conn = host->z.callback_rxcon;
                ...
                if (code2) {
                     ...
                     */
                    struct rx_connection *rxconn;
		    ViceLog(0,("CB: ProbeUuid for host %p (%s) failed %d\n",
			       host,
			       h_Host2str(host, &hoststr),
			       code2));
		    ...
		    host->z.addr.sin_family = AF_INET;
		    host->z.addr.sin_addr.s_addr = addr.sin_addr.s_addr;
		    host->z.addr.sin_port = addr.sin_port;
		    rxconn = host->z.callback_rxcon;
		...
		ViceLog(0,
			("CB: WhoAreYou failed for host %p (%s), error %d\n",
			 host, h_Host2str(host, &hoststr), code));
	...
	if (!(host->z.hostFlags & ALTADDR)) {
	    ...
	    ViceLog(125,
		    ("Host %p (%s) waiting for host-init to complete\n",
		     host, h_Host2str(host, &hoststr)));
	    h_Lock_r(host);
	    h_Unlock_r(host);
	    ViceLog(125,
	...
	if (!(host->z.Console & 1)) {
		...
		if (!pident)
		    rx_SetSpecific(tcon, rxcon_ident_key, identP);
		ViceLog(25,
			("Host %p (%s) does not support WhoAreYou.\n",
			 host, h_Host2str(host, &hoststr)));
		code = 0;
	    } else if (code == 0) {
		if (!identP)
		...
		if (oldHost) {
		    ...
		    if (!h_equal(&oldHost->z.addr, &addr)) {
			struct rx_connection *rxconn;
			ViceLog(25,
				 ("CB: Host %p (%s) has new addr %s\n",
				   oldHost,
				   h_Host2str(oldHost, &hoststr),
				   rx_Conn2str(tcon, &hoststr2)));
			/*
			 * add then remove.  otherwise the host may get marked
			 * deleted if we removed the only valid address.
			 */
			addInterfaceAddr_r(oldHost, &addr);
			/* Note: port number is not compared. */
			if (probefail || oldHost->z.addr.sin_addr.s_addr == addr.sin_addr.s_addr) {
			    /*
			     ...
			     * is behind a NAT.
			     */
			    removeInterfaceAddr_r(oldHost, &oldHost->z.addr);
			} else {
			    int i;
				...
				if (h_equal(&interface->interface[i].addr, &addr)) {
				    ...
				    taddr.sin_addr.s_addr = addr.sin_addr.s_addr;
				    taddr.sin_port = interface->interface[i].addr.sin_port;
				    removeInterfaceAddr_r(oldHost, &taddr);
				    break;
				}
			    }
			}
			oldHost->z.addr.sin_family = AF_INET;
			oldHost->z.addr.sin_addr.s_addr = addr.sin_addr.s_addr;
			oldHost->z.addr.sin_port = addr.sin_port;
		    ...
		    if (code == 0) {
			ViceLog(25,
				("InitCallBackState3 success on host %p (%s)\n",
				 host, h_Host2str(host, &hoststr)));
		    }
		}
	    }
	    if (code) {
		ViceLog(0,
			("CB: RCallBackConnectBack failed for %p (%s)\n",
			 host, h_Host2str(host, &hoststr)));
		host->z.hostFlags |= VENUSDOWN;
	    } else {
		ViceLog(125,
			("CB: RCallBackConnectBack succeeded for %p (%s)\n",
			 host, h_Host2str(host, &hoststr)));
...
h_FindClient_r(struct rx_connection *tcon, afs_int32 *a_viceid)
{
	...
	*a_viceid = viceid;
	...
	if (viceid == ANONYMOUSID) {
		...
		struct rx_inet_fmtbuf hoststr;
		ViceLog(0,
			("pr_GetCPS failed(%d) for user %d, host %p (%s)\n",
			 code, viceid, client->z.host,
			 h_Client2str(client, &hoststr)));
	...
	if (!oldClient->z.deleted) {
	    ...
	    ViceLog(0, ("FindClient: deleted client %p(%x ref %d host %p href "
			"%d) already had conn %p (host %s, cid %x), stolen "
...
/**
 * Format the netaddr:port of an AddrPort.
 *
 * @param[in]  interface pointer to a struct AddrPort
 * @param[out] buf       format work buffer
 *
 * @returns pointer to buffer with string representation of <netaddr>:<port>
 */
char *
h_AddrPort2str(struct AddrPort *interface, struct rx_inet_fmtbuf *buf)
{
    ...
    snprintf(buf->buffer, sizeof(buf->buffer), "%s:%hu",
	     rx_inet2str(interface->addr.sin_addr.s_addr, &inetfmtbuf),
	     ntohs(interface->addr.sin_port));
    return buf->buffer;
}
/**
 * Format the netaddr:port of a specific interface for a host.
 *
 * @param[in]  host    pointer to a struct host
 * @param[in]  iface_index   specific interface on host
 * @param[out] buf     format work buffer
 *
 * @returns pointer to buffer with string representation of <netaddr>:<port>
 */
char *
h_HostInterface2str(struct host *host, int iface_index,
		    struct rx_inet_fmtbuf *buf)
{
    if (iface_index < host->z.interface->numberOfInterfaces)
	return h_AddrPort2str(&host->z.interface->interface[iface_index], buf);
    else {
	strncpy(buf->buffer, "error:error", sizeof(buf->buffer));
	return buf->buffer;
    }
}

/**
 * Format the netaddr:port of a client.
 *
 * @param[in] client   pointer to a struct client
 * @param[out] buf     format work buffer
 *
 * @returns pointer to buffer with string representation of <netaddr>:<port>
 */
...
h_Client2str(struct client *client, struct rx_inet_fmtbuf *buf)
{
    ...
    snprintf(buf->buffer, sizeof(buf->buffer), "%s:%hu",
	    rx_inet2str(client->z.host->z.addr.sin_addr.s_addr, &inetfmtbuf),
	    ntohs(client->z.host->z.addr.sin_port));
    return buf->buffer;
}

/**
 * Format a netaddr:port of a host.
 *
 * @param[in] host     pointer to a struct host
 * @param[out] buf     format work buffer
 *
...
h_Host2str(struct host *host, struct rx_inet_fmtbuf *buf)
{
    ...
    snprintf(buf->buffer, sizeof(buf->buffer), "%s:%hu",
	     rx_inet2str(host->z.addr.sin_addr.s_addr, &inetfmtbuf),
	     ntohs(host->z.addr.sin_port));
    return buf->buffer;
}

/**
 * Format the netaddr:port, uuid and flags for a host.
 *
 * @param[in] host     pointer to a struct host
 * @param[out] buf     format work buffer
 *
 * @returns pointer to buffer with string representation of
 *                     <netaddr>:<port> uuid: <uuid> flags: <flags>
 */
char *
...
h_DumpHost(struct host *host, void *rock)
{
    ...
    H_LOCK;
    snprintf(tmpStr, sizeof tmpStr,
	     "host:%s hidx:%d cbid:%d lock:%x last:%u active:%u "
    ...
    if (host->z.interface)
	for (i = 0; i < host->z.interface->numberOfInterfaces; i++) {
	    struct rx_inet_fmtbuf ifacefmtbuf;
	    sprintf(tmpStr, " %s",
		     h_HostInterface2str(host, i, &ifacefmtbuf));
	    (void)STREAM_WRITE(tmpStr, strlen(tmpStr), 1, file);
	}
...
static int h_stateVerifyAddrHash(struct fs_dump_state * state, struct host * h,
                                 struct sockaddr_in *addr, int valid);
...
h_stateVerifyHost(struct host * h, void* rock)
{
    ...
    } else if (h_stateVerifyAddrHash(state, h, &h->z.addr, 1)) {
	state->bail = 1;
    }
                      ...
                      struct sockaddr_in *addr, int valid)
{
	...
	host = chain->hostPtr;
	if (host == NULL) {
	    afs_inet_ntoa_r(addr->sin_addr.s_addr, tmp);
	...
	if (state->mode == FS_STATE_LOAD_MODE) {
	    ViceLog(0, ("h_stateVerifyAddrHash: error: addr %s:%u not found in hash\n",
	                tmp, (unsigned)htons(addr->sin_port)));
	    ret = 1;
	    goto done;
	} else {
...
h_stateRestoreHost(struct fs_dump_state * state)
{
	...
	for (i = ifp->numberOfInterfaces-1; i >= 0; i--) {
		...
		&& !h_equal(&ifp->interface[i].addr, &host->z.addr)) {
...
{
    out->host = in->z.addr.sin_addr.s_addr;
    out->port = in->z.addr.sin_port;
...
{
    out->z.addr.sin_family = AF_INET;
    out->z.addr.sin_addr.s_addr = in->host;
    out->z.addr.sin_port = in->port;
...
{				/*h_GetHostNetStats */
    ...
    *a_diffNetworkP = (afs_int32) 0;
	...
	if (!(hostP->z.hostFlags & HOSTDELETED)) {
	     ...
	     */
	    (*a_numHostsP)++;
	    currAddr_HBO = (afs_uint32) ntohl(hostP->z.addr.sin_addr.s_addr);
...
initInterfaceAddr_r(struct host *host, struct interfaceAddr *interf)
{
    ...
    myAddr = host->z.addr.sin_addr.s_addr;	/* current interface address */
    myPort = host->z.addr.sin_port;		/* current port */

    ViceLog(125,
	    ("initInterfaceAddr : host %s numAddr %d\n",
	      h_Host2str(host, &hoststr), number));
	...
	interface->interface[count].addr.sin_family = AF_INET;
	interface->interface[count].addr.sin_addr.s_addr = myAddr;
	interface->interface[count].addr.sin_port = myPort;
        interface->interface[count].valid = 1;
    }

    for (i = 0; i < count; i++) {

	interface->interface[i].addr.sin_family = AF_INET;
	interface->interface[i].addr.sin_addr.s_addr = interf->addr_in[i];
	 ...
	 */
	interface->interface[i].addr.sin_port = port7001;
	...
	for (i = 0; i < host->z.interface->numberOfInterfaces; i++) {
	    ViceLog(125, ("--- alt address %s\n",
			  h_HostInterface2str(host, i, &hoststr)));
	}
    }
...
static int
h_DeleteHostFromAddrHashTable_r(struct sockaddr_in *addr,
				struct host *host)
{
    struct rx_inet_fmtbuf hoststr;
    ...
    ViceLog(125,
	    ("h_DeleteHostFromAddrHashTable_r: host %p (%s) not found\n",
	     host, h_Host2str(host, &hoststr)));
    return 0;
}
