...
UV_DeleteVolume(afs_uint32 aserver, afs_int32 apart, afs_uint32 avolid)
{
    ...
    conn = UV_Bind(aserver, AFSCONF_VOLUMEPORT);
    if (conn) {
	code = vs_DeleteVolume(conn, apart, avolid);
	rx_DestroyConnection(conn);
    }
    return code;
}
	       ...
	       afs_uint32 atoserver, afs_int32 atopart, int flags)
{
    ...
    if (fromconn && toconn) {
	code = vs_MoveVolume(afromvol, fromconn, afrompart, toconn, atopart, flags);
    }
    if (fromconn)
	       ...
	       afs_uint32 atovolid, int flags)
{
    ...
    toconn = UV_Bind(atoserver, AFSCONF_VOLUMEPORT);
    if (fromconn && toconn) {
	code = vs_CopyVolume(afromvol, fromconn, afrompart, atovolname,
			     toconn, atopart, atovolid, flags);
    }
    if (fromconn)
	rx_DestroyConnection(fromconn);
    if (toconn)
		 ...
		 afs_int32 afrompart, int flags)
{
    ...
    if (conn) {
	code = vs_ReleaseVolume(conn, afromvol, afrompart, flags);
	rx_DestroyConnection(conn);
    }
    return code;
}
		  ...
		  void *rock)
{
    ...
    if (conn) {
	code = vs_RestoreVolume(conn, topart, tovolid, toparentid,
...
UV_SyncVolume(afs_uint32 aserver, afs_int32 apart, char *avolname, int flags)
{
    ...
    /* Note: The server id argument is optional for this call. */
    if (aserver != 0)
	conn = UV_Bind(aserver, AFSCONF_VOLUMEPORT);
    code = vs_SyncVolume(conn, apart, avolname, flags);
    if (conn)
	rx_DestroyConnection(conn);
    return code;
}
...
UV_SyncVldb(afs_uint32 aserver, afs_int32 apart, int flags, int force)
{
    ...
    conn = UV_Bind(aserver, AFSCONF_VOLUMEPORT);
    if (conn) {
	code = vs_SyncVldb(conn, apart, flags);
	rx_DestroyConnection(conn);
    }
    return code;
}
...
UV_SyncServer(afs_uint32 aserver, afs_int32 apart, int flags, int force)
{
    ...
    conn = UV_Bind(aserver, AFSCONF_VOLUMEPORT);
    if (conn) {
	code = vs_SyncServer(conn, apart, flags);
	rx_DestroyConnection(conn);
    }
    return code;
}
