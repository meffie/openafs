...
char *servid2str(afs_uint32 serverId, struct serverName *name);
char *partid2str(afs_int32 partId, struct partName *name);
...
extern char *vs_GetServerName(struct rx_connection *conn, struct serverName *name);
extern char *vs_GetServerNameById(afs_uint32 serverId, struct serverName *name);
extern int vs_GetServerId(struct rx_connection *conn, afs_uint32 *serverId);
extern int vs_DeleteVolume(struct rx_connection *aconn,
			   afs_int32 apart, afs_uint32 avolid);
extern int vs_ListOneVolume(struct rx_connection *aconn,
...
extern int vs_AddVLDBEntry(afs_uint32 aserver, afs_int32 apart, char *aname,
			   afs_uint32 aid);
extern int vs_MoveVolume(afs_uint32 afromvol,
			 struct rx_connection *fromconn,
			 afs_int32 afrompart,
			 struct rx_connection *toconn,
			 afs_int32 atopart,
			 int flags);
extern int vs_BackupVolume(struct rx_connection *aconn,
			   afs_int32 apart, afs_uint32 avolid);
...
extern int vs_RestoreVolume(struct rx_connection *toconn,
...
extern int vs_ListPartitions(struct rx_connection *conn, struct partList *ptrPartList,
			     afs_int32 * cntp);
extern int vs_ZapVolumeClones(struct rx_connection *aconn, afs_int32 apart,
			      struct volDescription *volPtr,
			      afs_int32 arraySize);
extern int vs_GenerateVolumeClones(struct rx_connection *aconn,
				   afs_int32 apart, struct volDescription *volPtr,
				   afs_int32 arraySize);
extern int vs_ListVolumes(struct rx_connection *aconn, afs_int32 apart,
			  int all, struct volintInfo **resultPtr, afs_int32 * size);
extern int vs_XListVolumes(struct rx_connection *conn, afs_int32 a_partID,
			   int a_all, struct volintXInfo **a_resultPP,
			   afs_int32 * a_numEntsInResultP);
extern int vs_XListOneVolume(struct rx_connection *conn, afs_int32 a_partID,
			     afs_uint32 a_volID,
			     struct volintXInfo **a_resultPP);
extern int vs_SyncVolume(struct rx_connection *aconn, afs_int32 apart,
			 char *avolname, int flags);
extern int vs_SyncVldb(struct rx_connection *aconn, afs_int32 apart, int flags);
extern int vs_SyncServer(struct rx_connection *aconn, afs_int32 apart, int flags);
...
extern int vs_CopyVolume(afs_uint32 afromvol,
			 struct rx_connection *fromconn,
			 afs_int32 afrompart,
			 char *atovolname,
			 struct rx_connection *toconn,
			 ...
			 afs_uint32 atovolid,
			 int flags);
extern int vs_CloneVolume(struct rx_connection *aconn,
			  afs_int32 apart, afs_uint32 avolid,
...
extern int vs_ConvertRO(struct rx_connection *conn,
