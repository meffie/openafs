 ...
 * given volume type.  May be called repeatedly to get each read-only site in
 * the vldb entry when returns true.
 *
 ...
 *                        before first call with the entry
 *
 * @returns true (non-zero) when a volume site has been found.
 */
static int
